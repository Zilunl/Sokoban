...
bool dead_square_deadlock(sokoban_t* init_data, state_t* state)
{
    ...
    if (state->map[state->player_y + 1][state->player_x] == '$'){
        y = state->player_y + 1;
        deadlock = square_check(x, y, init_data, state);
    }
    if( state->map[state->player_y-1][state->player_x] == '$'){
        y = state->player_y - 1;
        deadlock = square_check(x, y, init_data, state);
	...
	if (!is_goal_loc( state->player_y, state->player_x, init_data)) {
		...
		if (state->map[y][x+1] == '#') {
			for (int i = 0; i < init_data->lines; i++) {
				for (int j = 0; state->map[i][j] != '\0'; j++) {
					if (state->map[i][j] == '.') {
						for (int a = 0; a < init_data->lines; a++) {
							if ((state->map[a][j] == '.') || (a != i)) {
								have_goals++;
							}
						}
						if (have_goals == 0) {
		...
		if (state->map[y+1][x] == '#') {
			for (int i = 0; i < init_data->lines; i++) {
				for (int j = 0; state->map[i][j] != '\0'; j++) {
					if (state->map[i][j] == '.') {
						for (int b = 0; state->map[i][b] != '\0'; b++) {
							if ((state->map[i][b] == '.') || (b != j)) {
								have_goals++;
							}
						}
						if (have_goals == 0) {
