...
void find_solution(sokoban_t* init_data, bool show_solution)
{
	...
	while (pq.size != 0) {
		...
		n = heap_delete(&pq);
		// Record n in to array
		expanded_nodes_table[expanded_nodes] = n;
		// Expand this array
		expanded_nodes = expanded_nodes + 1;

		// Find a solution, and save solution and size
		...
		for (a = 0; a < 4; a++){
			...
			// Check deadlock and free newnode
			if (!player_moved || simple_corner_deadlock(init_data, &(new_node->state))) {
				// Free new node and map
				for (int i = 0; i < init_data->lines; i++){
			...
			// Convert the map into a 1D map
			flatten_map(init_data, &flat_map, new_node->state.map);

			// Check duplicates in HashTable
			if ((ht_contains(&hashTable, flat_map)) != HT_NOT_FOUND) {
				duplicated_nodes = duplicated_nodes + 1;
				// Free newnode and map
				for (int i = 0; i < init_data->lines; i++){
					free(new_node->state.map[i]);
				}
				free(new_node->state.map);
	...
	// Free Memory of priority_queue and expanded_nodes_table
...
void solve(char const *path, bool show_solution)
{
	...
	*/
	...
	/**
	 * Free Memory of map, map_save and buffer
	*/
